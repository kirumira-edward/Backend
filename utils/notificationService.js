...
const sendPushNotification = async (notification) => {
  try {
    ...
    console.log(
      `Successfully sent push notifications: ${response.successCount}/${notification.deviceTokens.length}`
    );
    ...
    if (response.failureCount > 0) {
      ...
      // Log failed tokens
      console.log("Push notification failed for tokens:", failedTokens);
    }
  } catch (error) {
